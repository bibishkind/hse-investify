...
def get_info_by_ticker(ticker):
    json = requests.get(get_url_by_ticker(ticker)).json()
    return {
...
def get_url_by_ticker(ticker):
    json = requests.get(SEARCH_URL + ticker).json()
    symbol = json["symbols"][0]
    return INFO_URL + symbol["exchange"] + ":" + symbol["symbol"]
